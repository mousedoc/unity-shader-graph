Shader "Chimera/FluidColor_Update"
{
	...
	float4 solveFluid(sampler2D smp, float2 uv, float2 w, float time)
	{
		...
		float4 tr = tex2D(smp, uv + float2(w.x, 0));		// to right
		float4 tl = tex2D(smp, uv - float2(w.x, 0));		// to left
		float4 tu = tex2D(smp, uv + float2(0, w.y));		// to up
		float4 td = tex2D(smp, uv - float2(0, w.y));		// to down

		// delta 
		float3 dx = (tr.xyz - tl.xyz) * 0.5;				// left right blend 
